(function(){
  ...
  const JPEG_QUALITIES=[.82,.74,.66,.58,.50,.42,.34];
  ...
  async function prepareFourThreeImage(file){
    ...
    const widths=[Math.min(sourceWidth,800),...OUTPUT_WIDTHS]
    ...
    if(smallest&&smallest.blob.size<1_000_000) return smallest;
    throw new Error('Could not optimize this image on this device.');
  }
  ...
  function enhanceAddCard(){
    ...
    if(!card||card.dataset.imageHelpVersion===String(VERSION)) return;
    card.dataset.imageHelpVersion=String(VERSION);
    const workflow=card.querySelector('.admin-image-workflow');
    if(workflow){
      workflow.innerHTML='<strong>Image:</strong> Add the item first, then use <strong>Choose 4:3 image</strong>. You can select a 1536×1152 generated image directly; the browser will compress it automatically.';
    }
  }
  ...
  function updateScopeCopy(){
    ...
    const text='Full backend CRUD for menu items, including 4:3 image upload. Images update publicly after upload.';
  ...
  if(!document.getElementById('ling-menu-image-upload-styles-v5')){
    const style=document.createElement('style');
    style.id='ling-menu-image-upload-styles-v5';
    style.textContent=`
